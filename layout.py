...
def make_filter_column(index: int) -> dbc.Col:
    filter = dbc.Input(
    ...
    return dbc.Col(dbc.Row([dbc.Col(filter), dbc.Col(counter)]))
...
def create_layout(app: Dash) -> html.Div:
    sidebar_logo = html.A(
        ...
        children=[
            html.Img(
                src=app.get_asset_url("icon.png"), width=32, className="me-2"
            ),
            html.Span("sinc", className="fs-4"),
        ],
    )
    ...
    sidebar = html.Div(
        ...
        children=[
            ...
            dcc.Dropdown(
                id="num_per_page",
                options=[
                    {"label": str(i), "value": i} for i in [10, 20, 50, 100]
                ],
                value=10,
                className="mb-3",
            ),
        ...
        [
            html.Main(
                ...
                children=[
                    ...
                    html.Div(gallery, className="d-flex flex-grow-1"),
                ],
            ),
